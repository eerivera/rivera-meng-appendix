...
if __name__ == '__main__':
    ...
    print(min(orig_overheads), min(orig_parser_overheads))
    print(max(orig_overheads), max(orig_parser_overheads))
    print("Mean overhead - original (total and parser)")
    print(np.mean(orig_overheads))
    print(np.mean(orig_parser_overheads))
    print()
    
    fixed_before_overheads = [l.fixed_overhead for l in collectiondata]
    fixed_before_parser_overheads = [l.fixed_overhead for l in collectiondata if l.fn_name == "Parser::Parse"]

    print(min(fixed_before_overheads), min(fixed_before_parser_overheads))
    print(max(fixed_before_overheads), max(fixed_before_parser_overheads))
    print("Mean overhead - removing outlier runs per fn call (total and parser)")
    print(np.mean(fixed_before_overheads))
    print(np.mean(fixed_before_parser_overheads))
    print()

    fixed_after_parser_overheads = remove_outliers(orig_parser_overheads)
    fixed_after_overheads = remove_outliers([l.orig_overhead for l in collectiondata if l.fn_name != "Parser::Parse"]) + fixed_after_parser_overheads
    fixed_after_overheads_ignorant = remove_outliers(orig_overheads)

    print("Mean overhead - removing outlier fn calls (total and parser)")
    print(np.mean(fixed_after_overheads))
    print(np.mean(fixed_after_parser_overheads))
    print(np.mean(fixed_after_overheads_ignorant))
    print()

    fixed_before_after_parser_overheads = remove_outliers(fixed_before_parser_overheads)
    fixed_before_after_overheads = remove_outliers([l.fixed_overhead for l in collectiondata if l.fn_name != "Parser::Parse"]) + fixed_before_after_parser_overheads
    fixed_before_after_overheads_ignorant = remove_outliers(fixed_before_overheads)

    print("Mean overhead - removing outlier fn calls and outlier runs per fn call (total and parser)")
    print(np.mean(fixed_before_after_overheads))
    print(np.mean(fixed_before_after_parser_overheads))
    print(np.mean(fixed_before_after_overheads_ignorant))
    print()

    # true_orig_withoutmpk_sum = sum((i for l in collectiondata for i in l.orig_withoutmpk_values))
    # true_orig_withmpk_sum = sum((i for l in collectiondata for i in l.orig_withmpk_values))
    # true_overhead = true_orig_withmpk_sum / true_orig_withoutmpk_sum
    # true_orig_withoutmpk_sum_parser = sum((i for l in collectiondata for i in l.orig_withoutmpk_values if l.fn_name != "Parser::Parse"))
    # true_orig_withmpk_sum_parser = sum((i for l in collectiondata for i in l.orig_withmpk_values if l.fn_name != "Parser::Parse"))
    # true_overhead_parser = true_orig_withmpk_sum_parser / true_orig_withoutmpk_sum_parser
    # print("True overhead")
    # print(true_overhead)
    # print(true_overhead_parser)

    parser_calls = sum(l.fn_name == "Parser::Parse" for l in collectiondata)
    print(f"Parser calls: {parser_calls}")
    print(f"Total calls: {len(list(collectiondata))}")   
    
    average_time_by_call_name = {}
    for l in collectiondata:
        average_time_by_call_name.setdefault(l.fn_name, []).append(l.orig_withmpk_values.mean)

    for fn, vals in average_time_by_call_name.items():
        print(f"{fn}: {len(vals), np.mean(vals)}")
