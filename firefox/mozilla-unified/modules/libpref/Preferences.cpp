...
#define TIMEBEGIN auto starttime = __rdtscp()
#define TIMEEND(name) \
    auto endtime = __rdtscp(); \
    std::ofstream logfile("/opt/eerivera/firefox/logfiles/example-withmpk.csv", std::ios_base::app); \
    logfile << name << "," << endtime - starttime << "\n" \
...
nsPrefBranch::SetBoolPref(const char* aPrefName, bool aValue) {
  ...
  const PrefName& pref = GetPrefName(aPrefName);
  auto returnval = Preferences::SetBool(pref.get(), aValue, mKind);
  TIMEEND("prefs.setBoolPref");
  return returnval;
}
...
nsPrefBranch::SetCharPref(const char* aPrefName, const nsACString& aValue) {
  ...
  if (NS_FAILED(rv)) {
    TIMEEND("prefs.setCharPref");
    return rv;
  }
  auto returnval = SetCharPrefNoLengthCheck(aPrefName, aValue);
  TIMEEND("prefs.setCharPref");
  return returnval;
}
...
nsPrefBranch::SetIntPref(const char* aPrefName, int32_t aValue) {
  ...
  const PrefName& pref = GetPrefName(aPrefName);
  auto returnval = Preferences::SetInt(pref.get(), aValue, mKind);
  TIMEEND("prefs.setIntPref");
  return returnval;
}
...
nsPrefBranch::ClearUserPref(const char* aPrefName) {
  ...
  const PrefName& pref = GetPrefName(aPrefName);
  auto returnval = Preferences::ClearUser(pref.get());
  TIMEEND("prefs.clearUserPref");
  return returnval;
}
...
nsPrefBranch::PrefHasUserValue(const char* aPrefName, bool* aRetVal) {
  ...
  *aRetVal = Preferences::HasUserValue(pref.get());
  TIMEEND("prefs.prefHasUserValue");
  return NS_OK;
}
...
nsPrefBranch::LockPref(const char* aPrefName) {
  ...
  const PrefName& pref = GetPrefName(aPrefName);
  auto returnval = Preferences::Lock(pref.get());
  TIMEEND("prefs.lockPref");
  return returnval;
}
...
nsPrefBranch::UnlockPref(const char* aPrefName) {
  ...
  const PrefName& pref = GetPrefName(aPrefName);
  auto returnval = Preferences::Unlock(pref.get());
  TIMEEND("prefs.unlockPref");
  return returnval;
}
...
Preferences::ReadDefaultPrefsFromFile(nsIFile* aFile) {
  ...
  if (!aFile) {
    ...
    TIMEEND("prefs.readDefaultPrefsFromFile");
    return NS_ERROR_INVALID_ARG;
  }

  auto returnval = openPrefFile(aFile, PrefValueKind::Default);
  TIMEEND("prefs.readDefaultPrefsFromFile");
  return returnval;
}
...
Preferences::ReadUserPrefsFromFile(nsIFile* aFile) {
  ...
  if (!aFile) {
    ...
    TIMEEND("prefs.readUserPrefsFromFile");
    return NS_ERROR_INVALID_ARG;
  }

  auto returnval = openPrefFile(aFile, PrefValueKind::User);
  TIMEEND("prefs.readUserPrefsFromFile");
  return returnval;
}
...
Preferences::ResetPrefs() {
  ...
  auto returnval = InitInitialObjects(/* isStartup */ false);
  TIMEEND("prefs.resetPrefs");
  return returnval;
}
...
Preferences::SavePrefFile(nsIFile* aFile) {
  ...
  TIMEBEGIN;
  auto returnval = SavePrefFileInternal(aFile, SaveMethod::Asynchronous);
  TIMEEND("prefs.savePrefFile");
  return returnval;
}
...
Preferences::GetDefaultBranch(const char* aPrefRoot, nsIPrefBranch** aRetVal) {
  ...
  if (!aPrefRoot || !aPrefRoot[0]) {
    ...
    root.forget(aRetVal);
    TIMEEND("prefs.getDefaultBranch");
    return NS_OK;
  }
...
bool Preferences::GetBool(const char* aPrefName, bool aFallback,
                          PrefValueKind aKind) {
  TIMEBEGIN;
  auto returnval = Internals::GetPref(aPrefName, aFallback, aKind);
  TIMEEND("Preferences::GetBool");
  return returnval;
}
...
nsresult Preferences::SetBool(const char* aPrefName, bool aValue,
  ...
  auto returnval = pref_SetPref(nsDependentCString(aPrefName), PrefType::Bool, aKind,
                      prefValue,
                      /* isSticky */ false,
                      /* isLocked */ false,
                      /* fromInit */ false);
  TIMEEND("Preferences::SetBool");
  return returnval;
}
...
nsresult Preferences::SetInt(const char* aPrefName, int32_t aValue,
  ...
  auto returnval = pref_SetPref(nsDependentCString(aPrefName), PrefType::Int, aKind,
                      prefValue,
                      /* isSticky */ false,
                      /* isLocked */ false,
                      /* fromInit */ false);
  TIMEEND("Preferences::SetInt");
  return returnval;
}
...
nsresult Preferences::RemoveObserver(nsIObserver* aObserver,
  ...
  auto returnval = sPreferences->mRootBranch->RemoveObserver(aPref, aObserver);
  TIMEEND("prefs.removeObserver");
  return returnval;
}
